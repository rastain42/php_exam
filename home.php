<?php  
 ...
 $result = $mysqli->query("SELECT Title, Content, StartDate, Localisation, events.Id, users.Username FROM `events` INNER JOIN  users ON events.UserId = users.Id ORDER BY PostDate DESC; ");
          ...
          <div class="eventList">
               ...
               foreach($finalRows as $rows){
                    ...
                    foreach($rows as $row){
                         ...
                         echo '<button style="width:10vh; margin: 1vh" class="detailsBtn"><a style="color:black"href="details.php?id='.$row['Id'].'">Détails</a> </button>';
                         echo '</div>';
